...
class App extends Component {
	...
	async beginDownload() {
			if (this.state.downloadURL) {
				await this.setState({
						downloads: [ < DownloadComp onComplete = {
								() => this.next() }
							...
							updateTaskBarProgress = {
								(index, progress) => this.updateTaskBarValue(index, progress) }
			...
			componentDidMount() {
					...
					contact() {
							...
							this.alert( < Alert key = { new Date().toLocaleString() } header = { "About" } body = {
									...
									li > < a target = { "_blank" }
									onClick = {
										() => _electron.shell.openExternal("https://joshbrown.info/#contact") } > Joshua
									Brown < /a> <
									/li> <
									li > < a target = { "_blank" }
									onClick = {
										() => _electron.shell.openExternal("https://www.jacob-schneider.ga/contact.html") } > Jacob
									Schneider < /a> <
									/li> <
									br / >
									<
									...
									render() {
											return ( <
...

													{
														...
														<
														WindowFrame / > : null
													} { /* <-That won't render anything, but I don't want to delete the reference. */ }
													...
													<
													Tool shortcut = "+"
													onClick = { e => this.showPrompt() } icon = { "fas fa-plus" }
													/> <
													Tool shortcut = "*"
													onClick = {
														() => this.setState(prev => ({ settingsVisible: !prev.settingsVisible })) } icon = { "fas fa-cog" }
													/> <
													Tool onClick = {
														() => this.setState(prev => ({ pastDownloadsVisible: !prev.pastDownloadsVisible })) } icon = { "fas fa-clock" }
													/> {
														void(() => true)() ? (platform !== "darwin" ?
																...
																New Download <
																div className = { "accelerator" } > { platform === "darwin" ? "Cmd+N" : "Ctrl+N" } <
																/div> <
																/div> <
																div className = { "option" } onClick = {
																	() => this.pastDownloads() } >
																Show Past Downloads <
																div className = { "accelerator" } > { platform === "darwin" ? "Cmd+J" : "Ctrl+J" } <
																/div> <
																/div> <
																hr / >
																<
																div onClick = {
																	() => App.confirmExit() } className = { "option" } >
																Exit <
																div className = { "accelerator" } > { platform === "darwin" ? "Cmd+W" : "Ctrl+W" } <
																/div> <
																/div> <
																div onClick = {
																	() => this.setState(prev => ({ settingsVisible: !prev.settingsVisible })) } className = { "option" } >
																Open Settings <
																/div> <
																/div> <
																/div> <
																...
																<
																div className = { "option" } onClick = {
																	() => this.about() } > About < /div> <
																div className = { "option" } > Docs < /div> <
																div className = { "option" } onClick = {
																	() => _electron.shell.openExternal('https://github.com/jbis9051/quick_download/blob/master/LICENSE') } > Licensing Information <
																/div> <
																div className = { "option" } onClick = {
																	() => _electron.shell.openExternal('https://github.com/jbis9051/quick_download/issues/new') } > Report a bug <
																/div> <
																div className = { "option" } onClick = {
																	() => this.alert( < Alert key = { new Date().toLocaleString() } header = { "Contact us" } body = {
																			< ul >
																			<
																			li > < a
																			onClick = {
																...
																<
																div className = "downloads" > { this.state.downloads } <
																/div> { /* ------------------------------------------------------------------------------------------------New Download Prompt------------------------------------------------------------------------------------------------ */ } {
																	...
																	div className = { "suggestions" } > {
																			this.getDownloadNames().map((i, a) => < div key = { a } className = { "suggestion" + (this.state.currentSelection === a ? " focused" : "") } > < span onClick = {
																					() => this.acceptSuggestion(a) } > { i } < /span><br/ > < /div>)} <
																				/div> <
																				/div>
																				...
																				input onFocus = { field => this.setState({ focused: field.target }) } onBlur = {
																					() => this.setState({ focused: null }) } value = { this.state.downloadURL } onChange = {
																					e => void((() => {
																						if (this.state.stopSave)
																							this.setState({
																								stopSave: false
																				...
																				className = { "input_standard mousetrap url" } id = { "dl-url" } placeholder = { "Download URL" }
																				/> <
																				div className = { "suggestions" } > {
																					this.getDownloadUrls().map((i, a) => < div key = { a } onClick = {
																							() => this.acceptSuggestion(a) } className = { "suggestion" + (this.state.currentSelection === a ? " focused" : "") } >
																						<
																						span > { i } < /span><br/ > < /div>)} <
																						/div> <
																						/div> <
																						...
																						textarea onFocus = { field => this.setState({ focused: field.target }) } onBlur = {
																							() => this.setState({ focused: null }) } value = { this.state.customHeaders } onChange = {
																							e => void((() => {
																								/*
													 ...
													 this.setState({
													   ...
													   */
																						...
																						className = { "input_standard standard_code mousetrap url" } id = { "dl-headers" } placeholder = { '{"Cookie","token=quickdownloader"}' }
																						/> <
																						/div>
																						...
																						<
																						Tool className = { "confirm-btn" } icon = { "fas fa-check" } onClick = {
																							() => this.beginDownload() }
																						/> <
																						/div> <
																						/div> <
																						...
																						br / >
																							...
																							label onClick = {
																								() => this.changePath() } htmlFor = "save-location"
																						...
																						/div> {
																							(() => false)() ? ( <
																								...
																								input className = { "standard_radio right_aligned" } type = { "radio" } name = { "proxy-auth-type" } checked = { window.localStorage.getItem('proxySettings') === 'pac' } id = { "pac" } onChange = {
																									field => {
																										if (field.target.value === "on") {
...

																							{
																								...
																								label htmlFor = { "pac-location" } > Pac Script Location < /label></div >:
																									(window.localStorage.getItem('proxySettings') === "auth" ? ( <
																											...
																											input placeholder = { "proxy.example.com" } className = { "input_standard" } value = { window.localStorage.getItem('proxyHost') || "" } onChange = { field => void window.localStorage.setItem('proxyHost', field.target.value) || this.forceUpdate() } id = { "proxy-host" }
																											/>
																											...
																											br / >
																											<
																											input placeholder = { 8080 } className = { "inline_input" } type = { "number" } value = { window.localStorage.getItem('proxyPort') || "" } onChange = { field => void window.localStorage.setItem('proxyPort', field.target.value) || this.forceUpdate() } id = { "proxy-port" }
																											/> <
																											br / >
																											<
																											br / >
																											...
																											<
																											Checkbox checked = { window.localStorage.proxyRequiresCredentials === "true" } onChange = { value => (void window.localStorage.setItem("proxyRequiresCredentials", value)) || this.forceUpdate() } text = { "Proxy Requires Credentials" }
																											/>

																											{
																												(window.localStorage.proxyRequiresCredentials === "true" ?
																													...
																													div >
																													<
																													input placeholder = { "Proxy Username" } type = { "text" } className = { "input_standard" } onChange = { field => void(window.localStorage.proxyUsername = field.target.value) || this.forceUpdate() } value = { window.localStorage.proxyUsername || "" } id = { "proxy-username" }
																													/> <
																													input placeholder = { "Proxy Password" } type = { "password" } className = { "input_standard" } onChange = { field => void(window.localStorage.proxyPassword = field.target.value) || this.forceUpdate() } value = { window.localStorage.proxyPassword || "" } id = { "proxy-password" }
																													/> <
																													/div> : null)}
																										...
																										<
																										div className = { "box-display-area" } > { this.state.boxes } <
																											/div> <
																											/div>
																									);
																							}
																							...
																							render() {
																								return ( <
																									div className = { "checkbox" } onClick = {
																										() => (void this.setState(prev => ({ checked: !prev.checked }))) || this.props.onChange(!this.state.checked) } >
																									<
