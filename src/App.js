...
class App extends Component {
    ...
    getDownloadNames() {
        return JSON.parse(window.localStorage.downloadHistory || JSON.stringify([])).filter(i => (i.name || "").toLowerCase().indexOf((this.state.downloadName || "").toLowerCase()) >= 0 && (i.url || "").toLowerCase().indexOf((this.state.downloadURL || "").toLowerCase()) >= 0).map(i => i.name);
    }

    getDownloadUrls() {
        return JSON.parse(window.localStorage.downloadHistory || JSON.stringify([])).filter(i => (i.name || "").toLowerCase().indexOf((this.state.downloadName || "").toLowerCase()) >= 0 && (i.url || "").toLowerCase().indexOf((this.state.downloadURL || "").toLowerCase()) >= 0).map(i => i.url);
    }
