...
export default class DownloadComp extends React.Component {
    ...
    async startDownload() {
        ...
        const start = async () => {
            ...
            this.download.on('update', async info => {
    ...
    render() {
        return (
            <div
                className={"download" + (this.state.status === 1 ? " failed" : this.state.status === 2 ? " done" : (this.state.status === 3 ? " pending" : (this.state.status === 4 ? " awaiting" : "")))}>
                ...
                {this.state.details ?
                    <div className="download-details">
                        ...
                        <span className="download-detail"><b>Progress: </b>
							<span
                                className={"monospace"}> ({DownloadComp.calculateSize(this.state.completedBytes)} / {DownloadComp.calculateSize(this.state.size)}) {this.state.progress}%</span></span>
                    </div>
                    : undefined
                }
